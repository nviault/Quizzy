    ...
    <div className="min-h-screen flex flex-col justify-between">
      <header className="bg-black/30 backdrop-blur border-b border-white/10 px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setRole('PLAYER')}>
          <span className="text-3xl font-black text-yellow-400 tracking-wider">QUIZZY!</span>
          <span className="bg-quizzy-purple text-xs font-bold px-2 py-1 rounded text-white uppercase">Open Source</span>
        </div>
        ...
        <div className="flex bg-white/10 p-1 rounded-xl">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition ${
              role === 'PLAYER' ? 'bg-yellow-400 text-quizzy-darkPurple shadow' : 'text-gray-300 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition ${
              role === 'HOST' ? 'bg-yellow-400 text-quizzy-darkPurple shadow' : 'text-gray-300 hover:text-white'
            }`}
          >
      ...
      <footer className="text-center py-4 text-xs text-gray-400 border-t border-white/10">
        Quizzy Open Source sur Kubernetes — Conçu pour la performance et le temps réel.
      </footer>
    </div>
  );
};
